...
# ==============================================================================
# PAGE 9: POSITION SIZER (PRO VERSION WITH MO DOLLAR STOP)
# ==============================================================================
    ...
    # UPDATED TABS: ADDED MO DOLLAR STOP
    tab_new, tab_risk, tab_mo, tab_add, tab_trim = st.tabs(["🆕 Standard Entry", "🧮 Risk-Based Sizing", "🛡️ MO $Dollar Stop", "➕ Add (Pyramid)", "✂️ Trim (Sell Down)"])
...
# --- TAB 3: MO DOLLAR STOP (NEW) ---
    with tab_mo:
        st.subheader("🛡️ MO $Dollar Stop Strategy")
        st.markdown("**Core Principle:** Use the 'New Chart Stop' to calculate exactly how much you can scale in.")
        
        # Mode Selection
        mo_mode = st.radio("Mode", ["🆕 Plan New Trade", "🔗 Manage Active Campaign"], horizontal=True, key="mo_mode_sel")
        ...
        if mo_mode == "🆕 Plan New Trade":
            st.markdown("#### 1. Establish Hard Risk Budget")
            c1, c2, c3 = st.columns(3)
            man_eq = c1.number_input("Total Equity", value=equity, step=1000.0, key="mo_man_eq")
            man_rp = c2.number_input("Risk % Limit", value=0.50, step=0.1, key="mo_man_rp")
            calc_budget = man_eq * (man_rp / 100)
            c3.metric("Risk Budget ($)", f"${calc_budget:,.2f}")
            
            st.markdown("#### 2. Initial Position")
            
            # Ticker & Price Fetch Logic
            c_tick, c_ent, c_stop = st.columns(3)
            mo_plan_ticker = c_tick.text_input("Ticker", key="mo_plan_tick", placeholder="XYZ").upper()
            
            # Auto-Fetch Price if Ticker entered
            def_entry = 100.00
            if mo_plan_ticker:
                try: 
                    live_fetch = yf.Ticker(mo_plan_ticker).history(period='1d')['Close'].iloc[-1]
                    if live_fetch > 0: def_entry = float(live_fetch)
                except: pass
            
            calc_entry = c_ent.number_input("Entry Price ($)", value=def_entry, step=0.1, key="mo_man_ep")
            
            # Default stop 8% below entry for convenience
            def_stop = calc_entry * 0.92
            man_stop = c_stop.number_input("Stop Price ($)", value=float(def_stop), step=0.1, key="mo_man_sp")
            
            risk_per_share = calc_entry - man_stop
            max_shares = int(calc_budget / risk_per_share) if risk_per_share > 0 else 0
            
            st.write(f"**Max Shares allowed:** {max_shares}")
            calc_shares = st.slider("Select Initial Size", 0, int(max_shares*1.5), max_shares, key="mo_man_shs")
            
            # Results Calculation
            curr_risk = (calc_entry - man_stop) * calc_shares
            total_capital = calc_shares * calc_entry
            
            st.success(f"### Initial Trade Plan ({mo_plan_ticker if mo_plan_ticker else 'New Trade'})\n"
                       f"• **Size:** {calc_shares} shares\n"
                       f"• **Capital Committed:** ${total_capital:,.2f}\n"
                       f"• **Risk:** ${curr_risk:,.2f} (Budget: ${calc_budget:,.2f})")
            
        else: # LIVE CAMPAIGN MODE
            st.markdown("#### 1. Select Active Campaign")
            if not df_s.empty:
                ...
                if not open_ops.empty:
                    ...
                    sel_ticker_disp = sel_ticker
                    
                    row = open_ops[open_ops['Ticker'] == sel_ticker].iloc[0]
                    calc_shares = int(row['Shares'])
                    calc_entry = float(row['Avg_Entry'])
                    
                    # RETRIEVE BUDGET FROM DB
                    db_budget = row.get('Risk_Budget', 0.0)
                    
                    if db_budget > 0:
                        calc_budget = float(db_budget)
                        msg = "✅ Locked from Trade Log"
                    else:
                        calc_budget = equity * 0.005 # Fallback
                        msg = "⚠️ Legacy Trade (Using 0.5% Default)"
                    
                    # CALCULATE MAINTENANCE STOP
                    # Formula: (Shares * Stop) - Cost = -Budget
                    # Stop = (Cost - Budget) / Shares
                    total_cost_curr = calc_shares * calc_entry
                    maint_stop = (total_cost_curr - calc_budget) / calc_shares
                    
                    c1, c2, c3, c4 = st.columns(4)
                    c1.metric("Ticker", sel_ticker)
                    c2.metric("Locked Risk Budget", f"${calc_budget:,.2f}", msg)
                    c3.metric("Current Position", f"{calc_shares} shs", f"Avg: ${calc_entry:,.2f}")
                    c4.metric("Current Maint. Stop", f"${maint_stop:,.2f}", "To keep original risk")
                    
                else: st.info("No active campaigns.")
            else: st.error("Summary file empty.")

# --- SECTION 2: SCALE IN CALCULATOR (UPDATED) ---
        if mo_mode == "🔗 Manage Active Campaign" and calc_budget > 0 and calc_shares > 0:
            st.markdown("---")
            st.markdown(f"#### 🪜 Scale-In Calculator")
            
            # Auto-fetch live price
            live_add = calc_entry * 1.05
            try: live_add = yf.Ticker(sel_ticker_disp).history(period='1d')['Close'].iloc[-1]
            except: pass
            
            # 1. INPUTS
            sc1, sc2, sc3 = st.columns(3)
            add_price = sc1.number_input("Add Price ($)", value=float(live_add), step=0.1, key="mo_add_px")
            
            # Intelligent default for new stop
            def_new_stop = max(calc_entry, add_price * 0.95)
            new_chart_stop = sc2.number_input("New Chart Stop ($)", value=float(def_new_stop), step=0.1, key="mo_new_stop")
            
            # Target Size Input
            target_mode_mo = sc3.select_slider("Target Total Position", options=list(size_map.keys()), value="Full (10%)", key="mo_target_size")
            target_pct = size_map[target_mode_mo]

            # 2. THE MATH
            # A. Risk Math (Max Capacity)
            existing_risk_exposure = (calc_entry - new_chart_stop) * calc_shares
            new_risk_capacity = calc_budget - existing_risk_exposure
            new_risk_per_share = add_price - new_chart_stop
            
            if new_risk_per_share <= 0:
                 max_risk_shares = 0
                 risk_msg = "🚫 Stop > Price"
            else:
                 max_risk_shares = math.floor(new_risk_capacity / new_risk_per_share)
                 if max_risk_shares < 0: max_risk_shares = 0
            
            # B. Portfolio Math (Desired Target)
            target_equity_val = equity * (target_pct / 100)
            current_equity_val = calc_shares * add_price
            needed_val = target_equity_val - current_equity_val
            
            if needed_val <= 0:
                desired_shares = 0
            else:
                desired_shares = math.floor(needed_val / add_price)

            # C. The Verdict (Can we do it?)
            final_shares = min(desired_shares, max_risk_shares)
            is_limited_by_risk = max_risk_shares < desired_shares
            ...
            k1.metric("Desired Add", f"{desired_shares} shs", f"To reach {target_pct}%")
            
            risk_delta_color = "inverse" if is_limited_by_risk else "normal"
            k2.metric("Risk-Allowed Add", f"{max_risk_shares} shs", f"Cap: ${new_risk_capacity:,.0f} Risk", delta_color=risk_delta_color)
            
            if desired_shares == 0:
                k3.metric("Action", "HOLD", "Already at/over Target")
            elif is_limited_by_risk:
                k3.metric("Action", f"BUY {final_shares}", f"⚠️ Capped by Risk (Short {desired_shares - final_shares})", delta_color="inverse")
            else:
                k3.metric("Action", f"BUY {final_shares}", "✅ Full Target Approved")

            st.markdown("---")
            
            # ROW 2: Projections
            new_total_shares = calc_shares + final_shares
            
            p1, p2, p3 = st.columns(3)
            
            if new_total_shares > 0:
                new_total_cost = (calc_shares * calc_entry) + (final_shares * add_price)
                new_avg = new_total_cost / new_total_shares
                
                # Projected Weight
                proj_val = new_total_shares * add_price
                proj_weight = (proj_val / equity) * 100
                
                # Projected Risk
                proj_risk = (new_avg - new_chart_stop) * new_total_shares
                
                p1.metric("New Total Position", f"{new_total_shares} shs", f"Avg: ${new_avg:,.2f}")
                p2.metric("Projected Weight", f"{proj_weight:.1f}%", f"Target: {target_pct}%")
                
                risk_delta = f"${proj_risk - calc_budget:,.2f}"
                risk_color = "inverse" if proj_risk > calc_budget + 5 else "normal" # +5 buffer for rounding
                p3.metric("Total Risk Check", f"${proj_risk:,.2f}", f"vs Budget ${calc_budget:,.2f}", delta_color=risk_color)
                
                if new_chart_stop > calc_entry:
                     st.success(f"✅ **Free Roll:** New stop at ${new_chart_stop:.2f} locks in profit on orig shares to finance this add!")
                elif is_limited_by_risk:
                     st.warning(f"⚠️ **Risk Constraint:** You cannot reach {target_pct}% with a stop at ${new_chart_stop:.2f}. You would need a tighter stop to buy more.")
                else:
                     st.info(f"ℹ️ **Clean Add:** Scaling to {target_pct}% respects your ${calc_budget:,.0f} risk budget.")

    # --- TAB 4: PYRAMIDING (ADD ON) ---
    with tab_add:
        st.caption("Legacy Scale-In Calculator (Profit % Based)")
        ...
        if not open_positions.empty:
            ...
            if st.button("Calculate Add-On", key="add_btn"):
                ...
                if value_to_add <= 0:
                    st.error(f"You are already over the target weight!")
                else:
                    ...
                    with c_risk1: st.info(f"**Breakeven Stop:** ${be_stop:.2f}")
                    with c_risk2: st.info(f"**'Lock 50%' Stop:** ${house_money_stop:.2f}")

    # --- TAB 5: TRIM (SELL DOWN) ---
    with tab_trim:
