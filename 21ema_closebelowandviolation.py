...
for signal_ticker, trade_ticker in ticker_pairs:
    ...
    output_file = f"output/{signal_ticker.replace('^', '')}_{trade_ticker}_21ema_alternative_strategy_result.xlsx"

    # Step 1: Load and filter signal ticker data
    try:
        signal_df = pd.read_csv(signal_file)
    except FileNotFoundError:
        print(f"⚠️ Signal file not found: {signal_file}. Skipping {signal_ticker} -> {trade_ticker}.")
        continue
    ...
    try:
        ...
        print(f"⚠️ Trade file not found: {trade_file}. Skipping {signal_ticker} -> {trade_ticker}.")
        continue
    ...
    # Step 4: CORRECTED Alternative Strategy Signal Logic (using signal ticker)
    streak = 0
    in_wait_mode = False
    position_open = False
    last_buy_index = None
    ...
    print(f"✅ {signal_ticker} -> {trade_ticker} strategy complete. Saved to: {output_file}")
...
print("\n📊 Master summary saved to: output/21ema_alternative_strategy_summary.xlsx")