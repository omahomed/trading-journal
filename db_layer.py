        ...
        with conn.cursor() as cur:
            ...
            query = f"""
                SELECT
                    ...
                    {b1_max_select}
                    {mae_mfe_select}{sr8_activation_select}{declared_sr8_select}{peak_total_pl_select}{manual_price_select}s.be_stop_moved_at AS "BE_Stop_Moved_At",
...
def update_peak_total_pl(portfolio_name, trade_id, new_peak_pl):
    ...
    update_b1_max_return_pct — the peak-of-total-
...
def _save_detail_row_in_txn(cur, portfolio_id, row_dict):
    ...
    # Migration 049: Pyramid §2 Window rule exempt-reason capture.
    # Only add-on BUYs (A-series) beyond +15% from B1 declare a reason
    # ('sr8_rebuild' or 'fresh_base'). B1s / non-exempt adds / SELLs
