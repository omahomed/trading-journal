        ...
        <div className="wide">
          ...
          <div className="rule-card caution">
            ...
            <div className="rule-body">
              ...
              <div className="rule-effect">
                Opens the cycle. Exposure → 20 (step 0 credit). <b>Two-anchor model (2026-08-04):</b>
                <ul className="mt-1 ml-3 list-disc">
                  <li><span className="mono">rally_day_low</span> = the correction low (invalidation floor). Any intraday <span className="mono">low &lt; rally_day_low</span> fires RALLY_INVALIDATED.</li>
                  <li><span className="mono">rally_day_idx</span> = the correction-low bar; <span className="mono">rally_count</span> = days since the low (&quot;Day 1 of the rally = the low bar&quot;).</li>
                  <li><span className="mono">step0_bar_idx</span> = the STEP_0 bar itself. This is the <b>Day 1 anchor for FTD counting</b>. The FTD gate checks <span className="mono">(i − step0_bar_idx + 1) ≥ 4</span>, so the earliest FTD is 4 sessions from STEP_0.</li>
                </ul>
                <div className="mt-1">If STEP_0 fires the same bar as the low (upside-reversal case), all three anchor on the same bar. Otherwise, the rally low is Day 1 of the rally but Day 0 of FTD counting; STEP_0 the next day starts the FTD clock.</div>
              </div>
          ...
          <div className="rule-card caution">
            ...
            <div className="rule-body">
              <div className="rule-conds">
                ...
                <div className="cond"><span className="cond-label">On/after 2026-07-31</span> (<span className="mono">FTD_DUAL_INDEX_START</span>) — <b>Webby model (2026-08-04 rule replacement)</b>: fires when <b>either</b>:
                  <ul className="mt-1 ml-3 list-disc">
                    <li>IXIC close ≥ +1%, or</li>
                    <li>SPY close ≥ +1%.</li>
                  </ul>
                  <div className="mt-1"><b>Volume is not a gate.</b> Tracked in the signal meta as an informational annotation (<span className="mono">ixic_vol_up</span> / <span className="mono">spy_vol_up</span>) so the badge can render &quot;Volume ✓ / —&quot; alongside the FTD, but never blocks the fire. Aligns with Mike Webster&apos;s public position (WRO 57/58/59) that index-composite volume data is too noisy to gate on.</div>
                </div>
                <div className="cond"><span className="cond-label">Missing SPY data</span> falls back to IXIC-only rather than refusing — an unavailable SPY row shouldn&apos;t block a valid IXIC signal. Meta flags <span className="mono">spy_data_missing</span> so the badge can show the fallback state.</div>
              </div>
              <div className="rule-effect">Fires STEP_1_FTD. Stores <span className="mono">ftd_close</span> and <span className="mono">ftd_low</span> as invalidation anchors (<b>always IXIC&apos;s</b> — IXIC stays state authority for the soft-fail check regardless of which index confirmed). Exposure → 40 (step 0 + 1). Meta carries <span className="mono">confirmed_by</span> (<span className="mono">ixic</span> / <span className="mono">spy</span> / <span className="mono">both</span> / <span className="mono">ixic_legacy</span>) plus per-index volume annotations, rendered on the M Factor page.</div>
        ...
        <div className="wrap">
          ...
          <table className="constants">
            ...
            <tbody>
              ...
              <tr><td className="mono">FTD_DUAL_INDEX_START</td><td className="mono">2026-07-31</td><td>Cutover date: bars on/after use the dual-index Webby model — IXIC price ≥ +1% OR SPY price ≥ +1% fires. Volume is annotation only (never a gate). Pre-cutover bars use the legacy IXIC-price-only rule.</td></tr>
          ...
          <dl className="gloss">
            ...
            <dt>rally_day_low / rally_day_idx / step0_bar_idx</dt>
            <dd>Three anchors, do not conflate: <span className="mono">rally_day_low</span> = the correction low (invalidation floor — any subsequent intraday low below this invalidates the rally). <span className="mono">rally_day_idx</span> = the correction-low bar (drives <span className="mono">rally_count</span> = &quot;day of the rally&quot;). <span className="mono">step0_bar_idx</span> = the STEP_0 bar (drives the FTD gate: Day 1 of FTD counting starts here). When STEP_0 fires the same bar as the low, all three point at the same bar; otherwise the rally-low anchor lags STEP_0 by 1+ bars.</dd>
