...
import { GamePlanSection } from "./game-plan-section";
...
export function DailyJournal({ navColor, initialDate }: { navColor: string; initialDate?: string }) {
  ...
  // ref flips; the server enforces the editable window in POST
  // /api/journal/game-plan (see api/main.py:_game_plan_lock_date). The
  // client mirrors the same rule via isGamePlanEditable so a locked day
  // renders read-only and never fires a save.
  const [gamePlan, setGamePlan] = useState("");
  const gamePlanDirtyRef = useRef(false);
  ...
  // editable window; client also short-circuits when isGamePlanEditable
  // is false so a locked day never fires a save.
  useEffect(() => {
    if (!selectedDate) return;
    if (!gamePlanDirtyRef.current) return;
    if (!isGamePlanEditable(selectedDate, todayInChicago())) return;
    const handle = window.setTimeout(() => {
      void api.journalGamePlanSave(portfolio, selectedDate, gamePlan).then(res => {
        if ("error" in res) {
        ...
        log.error("daily-journal", "game_plan save failed", err);
        setGamePlanMsg({ ok: false, text: err instanceof Error ? err.message : String(err) });
      });
    }, 800);
    ...
    <div className="flex" style={{ animation: "slide-up 0.18s ease-out", minHeight: "100%" }}>
      ...
      <div className="flex-1 min-w-0 lg:pl-7">
        ...
        {/* Game Plan (migration 052) — pre-commit intent for the NEXT
            trading day. Rich-text (mirrors Daily Thoughts) when the
            day is editable per isGamePlanEditable(); read-only sanitized
            HTML display when locked. Sits between Checklist and Daily
            Scorecard per user spec 2026-07-26.
            Auto-save (debounced 800ms) fires from the effect above. */}
        <GamePlanSection
          value={gamePlan}
          onChange={(html) => { gamePlanDirtyRef.current = true; setGamePlan(html); }}
          editable={isGamePlanEditable(selectedDate, todayInChicago())}
          journalId={dayJournalId}
          portfolio={portfolio}
        />
        {gamePlanMsg && (
          <div className="mt-1 text-[11px] px-4" style={{ color: gamePlanMsg.ok ? "#16a34a" : "#e5484d" }}>
            {gamePlanMsg.ok ? "✓" : "✗"} Game Plan {gamePlanMsg.text}
          </div>
        )}
