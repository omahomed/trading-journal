...
import { NLVEntry } from "./nlv-entry";
...
describe("NLVEntry — multi-portfolio rendering", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    setupDefaultMocks();
  });
  ...
  test("renders one PortfolioCard per portfolio in the context", async () => {
    render(<NLVEntry navColor="#6366f1" />);
...
describe("NLVEntry — save handler", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    setupDefaultMocks();
  });
...
describe("NLVEntry — rally prefix re-fetches on date change", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    setupDefaultMocks();
  });
  ...
  test("calls rallyPrefix with the entry date and re-calls when the date changes", async () => {
    ...
    render(<NLVEntry navColor="#f59f00" />);
