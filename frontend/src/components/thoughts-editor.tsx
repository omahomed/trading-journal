...
// Phase 7: shared rich-text editor extracted from weekly-thoughts.tsx
// and parameterized so the daily report can mount an identical editor
// under a different title / storage key / upload endpoint. The Weekly
// Retro page consumes this via the thin <WeeklyThoughts> wrapper in
// weekly-thoughts.tsx; the Daily Report mounts <DailyThoughts>.
//
// Phase 3.5 history (preserved for context): builds on the Phase 3
// contentEditable + document.execCommand foundation, adding lists,
// indent/outdent, headings, blockquote/code/HR, an inline link popover
// (replacing the old window.prompt), functional font family/size
// dropdowns, video embed (YouTube/Vimeo), tables with a hover toolbar,
// and task lists with clickable checkboxes.
//
...
export type ThoughtsEditorEntityType = "weekly_retro" | "daily_journal";

export interface ThoughtsEditorProps {
  value: string;
  onChange: (html: string) => void;
  /** Discriminator: chooses the upload endpoint, R2 prefix allow-list,
   *  and inline-error copy. Required — there is no entity-agnostic
   *  default that's safe to ship. */
  entityType: ThoughtsEditorEntityType;
  /** Phase 4.1: required for inline image paste. When null, image-paste
   *  attempts surface a friendly "save first" inline error (matches
   *  TagPicker / SnapshotGallery's disabled-state idiom). When
   *  undefined, image paste is silently disabled — same UX as null. */
  entityId?: number | null;
  /** Phase 4.1: portfolio name forwarded to the upload endpoint. */
  portfolio?: string;
  /** SectionExpander chrome — title in the collapsed/expanded header. */
  title: string;
  /** localStorage key persisting the expanded/collapsed state. Each
   *  consumer passes its own so weekly + daily don't share state. */
  localStorageKey: string;
  /** SectionExpander bodyId. Used by the test suite to assert collapse
   *  toggling and by aria-controls on the header. */
  bodyId: string;
  /** Copy shown in the expander header when expanded. */
  expandedCaption: string;
  /** Placeholder text shown inside the editor when empty. */
  placeholderText: string;
  /** Aria-label on the contentEditable. */
  ariaLabel: string;
  /** Aria-label on the toolbar (e.g., "Weekly Thoughts formatting"). */
...
// Phase 4.1 — img src whitelist for inline images. Allows URLs from our
// R2 public bucket under either the weekly_retros/ prefix (Weekly Retro
// page) or the daily_journal/ prefix (Daily Report page, Phase 7). If
...
// one of the known path prefixes + an image extension (defense in
// depth). The hook treats this as a global allow-list — even though one
// editor instance only writes to one entity's R2 prefix, the hook can't
// see which instance the <img> came from on a paste sanitize pass.
...
const IMG_SRC_PATTERN = _R2_PUBLIC_URL
  ? new RegExp(
      ...
      "/(weekly_retros|daily_journal)/[\\w/.-]+\\.(png|jpe?g|gif|webp)(\\?[\\w=&,.-]*)?$",
    )
  : /^https:\/\/[\w.-]+\/(weekly_retros|daily_journal)\/[\w/.-]+\.(png|jpe?g|gif|webp)(\?[\w=&,.-]*)?$/i;
...
export function ThoughtsEditor({
  ...
  const uploadAndInsertImage = useCallback(async (file: File) => {
    ...
    try {
      // Phase 7: branch on entityType to pick the right upload endpoint.
      // Both return { view_url } so the post-upload swap is symmetric.
      const res = entityType === "weekly_retro"
        ? await api.uploadWeeklyThoughtsImage(entityId, file, portfolio)
        : await api.uploadDailyThoughtsImage(entityId, file, portfolio);
  ...
  }, [entityType, entityId, portfolio, showInlineError, emitChange, noEntityErrorMessage]);
  ...
  const handlePaste = useCallback((e: React.ClipboardEvent<HTMLDivElement>) => {
    ...
    emitChange();
  }, [emitChange, entityId, showInlineError, uploadAndInsertImage, noEntityErrorMessage]);
