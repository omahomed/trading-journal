...
/**
 * Mobile Journal Log — Phase 2 T2-1. Replaces the desktop 19-
 * column horizontal-scroll table with a chronological per-day
 * card stack. Read-only: tap a card → /daily-journal?date=… (the
 * existing desktop detail view, which gets its own mobile-fit in
 * T2-4).
 *
 * Data source mirrors desktop journal-log.tsx: single
...
// rest of the mobile chrome. Desktop component is at journal-log
...
// 5-bucket grade label mirrors desktop journal-log.tsx:338.
// Distinct from Daily Journal's 11-bucket letterGrade — different
...
export function MobileJournalLog() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err) => {
        log.error("mobile-journal-log", "journalHistory fetch failed", err);
        if (!cancelled) {
          setHistory([]);
          setLoading(false);
        }
  ...
  const handleTap = (day: string) => {
    router.push(`/daily-journal?date=${day}`);
  };
...
// shared landing surface for Weekly Retro + NLV Entry without
...
function DailyNavRow({
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        data-testid="daily-nav-nlv-entry"
