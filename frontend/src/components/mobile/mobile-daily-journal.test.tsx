...
// Mirrors more.test.tsx + mobile-daily-journal.test.tsx pattern.
...
describe("MobileDailyJournal — mount fetch", () => {
  test("fires all parallel fetches with active portfolio + date", async () => {
    render(<MobileDailyJournal initialDate="2026-05-25" />);
    await waitFor(() => {
...
describe("MobileDailyJournal — no journal entry", () => {
  test("redirects to /journal-log after fetch resolves with no matching row", async () => {
    vi.mocked(api.journalHistory).mockResolvedValue([
      journalFixture({ day: "2026-04-01", id: 1 }),
    ]);
    render(<MobileDailyJournal initialDate="2026-05-25" />);
    await waitFor(() =>
      expect(replaceMock).toHaveBeenCalledWith("/journal-log"),
    );
  });

  test("disabled-state pill button routes to /journal-log unconditionally", async () => {
    ...
    expect(pushMock).toHaveBeenCalledWith("/journal-log");
    expect(backMock).not.toHaveBeenCalled();
  });
});

describe("MobileDailyJournal — invalid date redirect", () => {
  test("undefined initialDate fires router.replace('/journal-log')", async () => {
  ...
  test("malformed initialDate fires router.replace('/journal-log')", async () => {
    render(<MobileDailyJournal initialDate="not-a-date" />);
    await waitFor(() =>
      expect(replaceMock).toHaveBeenCalledWith("/journal-log"),
    );
  });
...
describe("MobileDailyJournal — header meta cycle label", () => {
...
describe("MobileDailyJournal — header back nav", () => {
  test("back chevron routes to /journal-log unconditionally", async () => {
    vi.mocked(api.journalHistory).mockResolvedValue([
...
describe("MobileDailyJournal — drawdown", () => {
...
describe("MobileDailyJournal — daily $ delta in metrics tile", () => {
...
describe("MobileDailyJournal — focus mode hydration", () => {
...
describe("MobileDailyJournal — Daily Recap preview", () => {
...
describe("MobileDailyJournal — Daily Thoughts preview", () => {
...
describe("MobileDailyJournal — Recap edit sheet save flow", () => {
...
describe("MobileDailyJournal — Performance Comparison", () => {
...
describe("MobileDailyJournal — Positions Opened", () => {
...
describe("MobileDailyJournal — Positions Closed", () => {
...
describe("MobileDailyJournal — Captures + EOD gallery", () => {
...
describe("MobileDailyJournal — Tags", () => {
...
describe("MobileDailyJournal — grade tile", () => {
  test.each([
    [5, "A+"],
    [4, "A"],
    [3, "B"],
...
describe("MobileDailyJournal — DAILY P&L label", () => {
...
describe("MobileDailyJournal — Market Notes header line", () => {
