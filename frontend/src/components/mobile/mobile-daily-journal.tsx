 ...
 * the desktop daily-journal.tsx (789 LOC, multi-section detail
 ...
 *   - Drawdown formula + thresholds match daily-journal.tsx exactly.
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      if (res && typeof res === "object" && "status" in res && res.status !== "ok") {
        log.error("mobile-daily-journal", `journalEdit non-ok for ${field}`, res);
        return;
      }
      onSaveSuccess(value);
      ...
      log.error("mobile-daily-journal", `journalEdit threw for ${field}`, err);
    } finally {
      setIsSaving(false);
    }
...
// Score → letter grade (mirrors daily-journal.tsx:577).
...
export function MobileDailyJournal({ initialDate, navColor = "#f59f00" }: Props) {
  ...
  // values redirect to /journal-log so users land on the list and
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.journalHistory(portfolio, 0).catch((err) => {
        log.error("mobile-daily-journal", "journalHistory failed", err);
        return [] as JournalHistoryPoint[];
      }),
      api.listEodSnapshots(date, portfolio).catch((err) => {
        log.error("mobile-daily-journal", "listEodSnapshots failed", err);
        return [] as EodRow[];
      }),
      api.tradesRecent(portfolio, 500).catch((err) => {
        log.error("mobile-daily-journal", "tradesRecent failed", err);
        return { details: [], lot_closures: [] };
      }),
      api.tradesClosed(portfolio, 500).catch((err) => {
        log.error("mobile-daily-journal", "tradesClosed failed", err);
        return [] as TradePosition[];
      }),
      api.listTags(portfolio).catch((err) => {
        log.error("mobile-daily-journal", "listTags failed", err);
        return [] as Tag[];
      }),
  ...
  // redirect to /journal-log. The dead-end DisabledState is kept as
  ...
  // All back-nav handlers route unconditionally to /journal-log.
...
function LoadedReport({
  ...
  // ── Drawdown (mirrors daily-journal.tsx:320-327) ─────────────
  const ddPct = useMemo(() => {
  ...
  // spy / nasdaq price columns (daily-journal.tsx:341-342). YTD
  // uses first-of-year vs current row for SPY/NDX, chained portfolio
  // daily_pct_change for portfolio (daily-journal.tsx:310-316).
  const perf = useMemo(() => {
    ...
    <div className="flex flex-col gap-1 px-5 pt-2">
      <div className="flex items-center justify-between gap-3">
        <button
          type="button"
          onClick={onBack}
          aria-label="Back to Journal Log"
      ...
      {/* Market Notes one-liner — third line below the meta strip. When
          ...
          an entry point without having to open the full Daily Journal. */}
      <div
 ...
 *  same chain the desktop daily-journal.tsx uses (and the mobile
    ...
    <div className="flex flex-col gap-3 pt-2">
      ...
      >
        ...
        <div className="text-[12px] text-m-text-dim">
          Log NLV for this date first, or create the entry on desktop.
        </div>
        <button
          type="button"
          onClick={onBack}
