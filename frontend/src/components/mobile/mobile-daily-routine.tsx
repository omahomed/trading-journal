 ...
 * the desktop daily-routine.tsx (789 LOC, multi-section detail
 ...
 *   - Drawdown formula + thresholds match daily-routine.tsx exactly.
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      if (res && typeof res === "object" && "status" in res && res.status !== "ok") {
        log.error("mobile-daily-routine", `journalEdit non-ok for ${field}`, res);
        return;
      }
      onSaveSuccess(value);
      try {
      ...
      log.error("mobile-daily-routine", `journalEdit threw for ${field}`, err);
    } finally {
      setIsSaving(false);
    }
...
// Score → letter grade (mirrors daily-routine.tsx:577).
...
// Drawdown color tiers — mirrors daily-routine.tsx:336-337 exactly.
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.journalHistory(portfolio, 0).catch((err) => {
        log.error("mobile-daily-routine", "journalHistory failed", err);
        return [] as JournalHistoryPoint[];
      }),
      api.listEodSnapshots(date, portfolio).catch((err) => {
        log.error("mobile-daily-routine", "listEodSnapshots failed", err);
        return [] as EodRow[];
      }),
      api.tradesRecent(portfolio, 500).catch((err) => {
        log.error("mobile-daily-routine", "tradesRecent failed", err);
        return { details: [], lot_closures: [] };
      }),
      api.tradesClosed(portfolio, 500).catch((err) => {
        log.error("mobile-daily-routine", "tradesClosed failed", err);
        return [] as TradePosition[];
      }),
      api.listTags(portfolio).catch((err) => {
        log.error("mobile-daily-routine", "listTags failed", err);
        return [] as Tag[];
      }),
...
function LoadedReport({
  ...
  // ── Drawdown (mirrors daily-routine.tsx:320-327) ─────────────
  const ddPct = useMemo(() => {
  ...
  // spy / nasdaq price columns (daily-routine.tsx:341-342). YTD
  // uses first-of-year vs current row for SPY/NDX, chained portfolio
  // daily_pct_change for portfolio (daily-routine.tsx:310-316).
  const perf = useMemo(() => {
 ...
 *  same chain the desktop daily-routine.tsx uses (and the mobile
