...
import { MobileJournalLog } from "./mobile-journal-log";
...
describe("MobileJournalLog — filter chips", () => {
...
describe("MobileJournalLog — sticky month headers", () => {
...
describe("MobileJournalLog — card tap navigation", () => {
  test("tapping a card pushes to /daily-journal?date=YYYY-MM-DD", async () => {
    const day = todayStr();
    vi.mocked(api.journalHistory).mockResolvedValue([entryFixture({ day })]);
    render(<MobileJournalLog />);
    const card = await screen.findByTestId(`day-card-${day}`);
    fireEvent.click(card);
    expect(pushMock).toHaveBeenCalledWith(`/daily-journal?date=${day}`);
  });
});

describe("MobileJournalLog — grade pill tier mapping", () => {
...
describe("MobileJournalLog — MCT badge", () => {
...
describe("MobileJournalLog — sub-row layout (4 items, no Heat)", () => {
...
describe("MobileJournalLog — empty states", () => {
...
describe("MobileJournalLog — end-of-list footer", () => {
...
describe("MobileJournalLog — T2-6 Daily nav row", () => {
  test("renders Weekly Retro + NLV Entry quick-link buttons", async () => {
    vi.mocked(api.journalHistory).mockResolvedValue([
      entryFixture({ day: todayStr() }),
    ]);
    render(<MobileJournalLog />);
    ...
    const nlv = within(row).getByTestId("daily-nav-nlv-entry");
    expect(weekly).toHaveTextContent("Weekly Retro");
    expect(nlv).toHaveTextContent("NLV Entry");
  });
  ...
  test("NLV Entry button routes to /nlv-entry", async () => {
    vi.mocked(api.journalHistory).mockResolvedValue([
      entryFixture({ day: todayStr() }),
    ]);
    render(<MobileJournalLog />);
    fireEvent.click(await screen.findByTestId("daily-nav-nlv-entry"));
    expect(pushMock).toHaveBeenCalledWith("/nlv-entry");
  });
  ...
  test("nav row renders between entries-count subtitle and Week/Month/All filter pills", async () => {
    ...
    const { container } = render(<MobileJournalLog />);
