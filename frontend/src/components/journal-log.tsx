...
type MctState = {
  ...
  // into the journal row at NLV Entry save time (migration 015) — the
...
function exportCsv(rows: JournalHistoryPoint[]) {
  ...
  a.href = url;
  a.download = `journal-log-${getActivePortfolio()}-${stamp}.csv`;
...
export function JournalLog({ navColor }: { navColor: string }) {
  ...
  // mct_display_day_num, snapshotted at NLV Entry save time per migration
                    ...
                    return (
                      <tr key={i} className="transition-colors"
                        ...
                        <td className="px-2.5 py-2 whitespace-nowrap" style={{ fontFamily: mono, fontSize: 10, color: "var(--ink-4)", cursor: "context-menu" }}
                            title="Right-click to open Daily Journal"
                            onContextMenu={e => { e.preventDefault(); router.push(`/daily-journal?date=${dateKey}`); }}>
                          {dateKey}
                        </td>
                        ...
                        <td className="px-2.5 py-2 whitespace-nowrap text-right" style={{ fontFamily: mono }}>
                          {(() => {
                            const tc = (h as any).trend_count;
                            if (tc == null) return <span style={{ color: "var(--ink-5)" }}>—</span>;
                            const color = tc > 0 ? "#08a86b" : tc < 0 ? "#e5484d" : "var(--ink)";
                            return <span style={{ color }}>{tc > 0 ? `+${tc}` : tc}</span>;
                          })()}
                        </td>
                          ...
                          return (
                            <>
                              <td className="px-2.5 py-2 text-center" style={{ cursor: "context-menu" }}
                                  title="Right-click to open Daily Journal"
                                  onContextMenu={e => { e.preventDefault(); router.push(`/daily-journal?date=${String(h.day).slice(0, 10)}`); }}>
                                {gradeLabel && (
