...
# --- TAB 3: MO DOLLAR STOP (NEW) ---
    with tab_mo:
        ...
        # --- SECTION 2: SCALE IN CALCULATOR (ONLY FOR ACTIVE CAMPAIGNS) ---
        if mo_mode == "🔗 Manage Active Campaign" and calc_budget > 0 and calc_shares > 0:
            ...
            if new_risk_per_share <= 0:
                 st.error("🚫 Stop Price must be below Add Price.")
                 max_add_shares = 0
            else:
                 max_add_shares = math.floor(new_risk_capacity / new_risk_per_share)
                 if max_add_shares < 0: max_add_shares = 0

            # DISPLAY RESULTS
            st.markdown("### 🎯 Execution Parameters")
            
            k1, k2, k3 = st.columns(3)
            k1.metric("New Buying Power", f"{int(max_add_shares)} shares", f"Risk Cap: ${new_risk_capacity:,.0f}")
            
            new_total_shares = calc_shares + max_add_shares
            k2.metric("New Total Position", f"{new_total_shares} shs", f"Value: ${(new_total_shares*add_price):,.0f}")
            
            if new_total_shares > 0:
                total_cost = (calc_shares * calc_entry) + (max_add_shares * add_price)
                new_avg = total_cost / new_total_shares
                total_risk_check = (new_avg - new_chart_stop) * new_total_shares
                
                k3.metric("Total Risk Check", f"${total_risk_check:,.2f}", f"Budget: ${calc_budget:,.2f}")
                
                if total_risk_check > calc_budget + 1.0: 
                     st.warning("⚠️ Math Warning: Risk slightly exceeds budget.")
                elif new_chart_stop > calc_entry:
                     st.success(f"✅ **Free Roll:** Your new stop (${new_chart_stop:.2f}) locks in profit on the original shares, financing this add!")
                else:
                     st.info(f"ℹ️ **Standard Pyramid:** Raising stop to ${new_chart_stop:.2f} keeps total risk at ${calc_budget:.0f}.")
            else:
                st.warning("⚠️ **No Room to Add:** Your new stop is too loose. You would exceed your risk budget immediately.")
        ...
        if not df_s.empty:
             ...
             if not df_open.empty:
                 ...
                 def get_trend_status(row):
                     ...
                     try:
                         ...
                         if not hist.empty:
                             ...
                             # 2. CHECK 10% RULE
                             target_10 = avg_cost * 1.10
                             if row['Current Price'] >= target_10:
                                 # Check if Stop is still loose (Below Breakeven)
                                 if curr_stop < avg_cost:
                                     return "🔔 HIT +10% (Move Stop to BE!)"
                                 else:
                                     return "✅ Secured (Stop @ BE+)"
                             
                             return f"⏳ Building (21e: ${live_ema:.2f})"
                     except: return "N/A"
                     return "N/A"
                 ...
                 for idx, row in df_open.iterrows():
                     ...
                     # 1. Budget Violation (Buffer 1.0)
                     if budget > 0:
                         if curr_risk > (budget + 1.0):
                             excess = curr_risk - budget
                             pct_over = (curr_risk / budget) * 100
                             warnings.append(f"⚠️ **{row['Ticker']}**: Capital Risk (${curr_risk:,.0f}) exceeds Budget (${budget:,.0f}) by ${excess:,.0f} ({pct_over:.0f}%). **Raise Stop.**")
                             all_clear = False
                     
                     # 2. Drawdown Violation (>7%)
                     ret = row['Return_Pct']
                     if ret <= -7.0:
                         warnings.append(f"🔴 **{row['Ticker']}**: Down {ret:.2f}%. Violates 7-8% Stop Loss Rule. **Immediate Action Required.**")
                         all_clear = False

                     # 3. 10% Rule Reminder (From Trend Status)
                     status = row.get('Trend Status', '')
                     if "HIT +10%" in status:
                         warnings.append(f"🔔 **{row['Ticker']}**: Stock hit +10%. **Move Stop to Breakeven.**")
                         all_clear = False
                
                 if all_clear:
                     st.success("✅ **ALL CLEAR:** System Health Good. All positions are sized within their Risk Budgets, and no 7% violations found.")
                 else:
                     for w in warnings:
                         if "Violates" in w: st.error(w)
                         elif "HIT +10%" in w: st.info(w)
                         else: st.warning(w)
