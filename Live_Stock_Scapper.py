...
def fetch_latest_data(symbol, df):
    ...
    if not df.empty:
        ...
        if last_date.date() == today.date():
            print(f"   ✓ Already have today's data ({today.strftime('%Y-%m-%d')})")
            return df
    
    print(f"   📊 Fetching today's real-time data...")
    ...
    try:
        ...
        if any(v is None for v in today_data.values()):
            print("   🔄 Info incomplete, trying history method...")
            ...
            if not recent_hist.empty:
                ...
                if latest_date == today.date():
                    ...
                    print("   🔄 Today not in history, trying real-time quote...")
        ...
        if today_data['Close'] is not None and today_data['Open'] is not None:
            ...
            print(f"   ✅ Added today's data (as of {current_time}):")
            ...
            print(f"   ⚠️  Could not fetch complete data for today")
            
    except Exception as e:
        print(f"   ⚠️  Error fetching today's data: {e}")
            ...
            if not recent.empty:
                for idx in reversed(range(len(recent))):
                    if recent.index[idx].date() == today.date():
                        ...
                        today_df = pd.DataFrame([today_data])
                        df = pd.concat([df, today_df], ignore_index=True)
                        print(f"   ✅ Found today's data in recent history")
                        break
        except:
            pass
...
for symbol in tickers:
    ...
    if os.path.exists(output_path):
        print(f"🔄 Overriding existing file: {output_path}")
...
if now.hour < 16:
    print(f"\n⏰ Note: Market is still open. Data includes current values as of {now.strftime('%I:%M %p')}")
else:
    print(f"\n✅ Market closed. Today's final data included.")
