...
def test_mirror_helper_writes_earliest_buy_fields(monkeypatch):
    ...
    Migration 047 added `rules` (JSONB array) to the earliest-BUY SELECT
    and mirrors it onto trades_summary alongside rule/buy_notes/stop_loss.
    Post-047 the fetched tuple is 4-wide and the UPDATE binds an extra
    rules parameter (JSON-serialized) between stop_loss and the WHERE
    portfolio_id.
    """
    fetches = [
        (1,),                                 # portfolio_id
        ("br1.3 Cup w/o Handle",
         "Initial entry on breakout", 195.0,
         ["br1.3 Cup w/o Handle"]),           # earliest BUY (rule, notes, stop, rules)
    ...
    # params: (rule, notes, stop_loss, rules_json, portfolio_id, trade_id)
    assert params == ("br1.3 Cup w/o Handle", "Initial entry on breakout",
                      195.0, '["br1.3 Cup w/o Handle"]',
                      1, "202605-001")
...
def test_mirror_helper_skips_sell_update_when_open(monkeypatch):
    ...
    fetches = [
        (1,),                                  # portfolio_id
        ("X", "n", 100.0, ["X"]),              # earliest BUY (post-047: 4-tuple)
...
def test_mirror_helper_writes_sell_fields_when_closed(monkeypatch):
    ...
    fetches = [
        (1,),                                            # portfolio_id
        ("br1", "buy notes", 100.0, ["br1"]),  # earliest BUY (post-047: 4-tuple)
...
def test_mirror_helper_sanitizes_nan_via_clean_text(monkeypatch):
    ...
    fetches = [
        (1,),                          # portfolio_id
        (np.nan, "nan", 195.0, []),    # earliest BUY — both pollution forms (post-047: 4-tuple)
        None,                          # no SELL
    ]
    conn, cur = _make_conn(fetches)
    monkeypatch.setattr(db_layer, "get_db_connection",
...
def test_b2_edit_does_not_change_summary_when_b1_earlier(monkeypatch):
    ...
    fetches = [
        (1,),
        ("X", "B1 notes", 195.0, ["X"]),  # earliest BUY (B1) — UNCHANGED by edit to B2
        None,
    ]
    conn, cur = _make_conn(fetches)
    monkeypatch.setattr(db_layer, "get_db_connection",
...
def test_b1_edit_promotes_new_rule_when_b2_exists(monkeypatch):
    ...
    fetches = [
        (1,),
        ("Z", "B1 notes updated", 195.0, ["Z"]),  # earliest BUY (B1) post-edit
        None,
    ]
    conn, cur = _make_conn(fetches)
    monkeypatch.setattr(db_layer, "get_db_connection",
...
def test_partial_sell_edit_does_not_mirror(monkeypatch):
    ...
    fetches = [
        (1,),
        ("X", "B1 notes", 195.0, ["X"]),  # earliest BUY (post-047: 4-tuple)
