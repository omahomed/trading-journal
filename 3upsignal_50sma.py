...
print(f"📊 3 Up Signal Strategy Tester (SMA50)")
print(f"📈 Using 50-period Simple Moving Average")
...
for ticker in TICKERS:
    ...
    # Calculate SMA50 (Simple Moving Average)
    df["SMA50"] = df["Close"].rolling(window=50).mean()
    
    # Generate signals using 3 Up Signal logic with SMA50
    df["signal"] = 0
    position_open = False
    first_break_low = None
    
    for i in range(2, len(df)):
        # Skip if SMA50 is not calculated yet (needs 50 periods)
        if pd.isna(df.iloc[i]["SMA50"]):
            continue
            
        # BUY SIGNAL: 3 consecutive days with low > SMA50 AND current day is up day
        if not position_open and i >= 2:
            # Check if SMA50 is available for all 3 days
            if not pd.isna(df.iloc[i-2]["SMA50"]) and not pd.isna(df.iloc[i-1]["SMA50"]):
                three_days_above = (df.iloc[i]["Low"] > df.iloc[i]["SMA50"] and 
                                   df.iloc[i-1]["Low"] > df.iloc[i-1]["SMA50"] and 
                                   df.iloc[i-2]["Low"] > df.iloc[i-2]["SMA50"])
        ...
        # SELL SIGNAL: Two-step process
            ...
            # Step 1: Check if we closed below SMA50 for first time
            if first_break_low is None:
                if df.iloc[i]["Close"] < df.iloc[i]["SMA50"]:
                    if i > 0 and not pd.isna(df.iloc[i-1]["SMA50"]):
                        if df.iloc[i-1]["Close"] >= df.iloc[i-1]["SMA50"]:
...
# === Simulate 3 Up Signal Strategy with SMA50 ===
print("\n🔄 Running 3 Up Signal strategy simulation with SMA50...")
...
if all_trades:
    ...
    with pd.ExcelWriter("output/3up_signal_SMA50_results.xlsx", engine='openpyxl') as writer:
    ...
    print(f"\n💾 Results saved to: output/3up_signal_SMA50_results.xlsx")
    
    # === Print Summary to Console ===
    print("\n" + "="*60)
    print("📊 3 UP SIGNAL STRATEGY RESULTS (SMA50)")
    print("="*60)
    print(f"Indicator: 50-period Simple Moving Average")
    print(f"Test Period: {PORTFOLIO_START_DATE} to {END_DATE}")
    print(f"Position Size: ${POSITION_SIZE:,.2f}")
    ...
    print("  3. The 3 Up Signal conditions were met with SMA50")
    print("  4. There's enough historical data (SMA50 needs 50 periods)")
