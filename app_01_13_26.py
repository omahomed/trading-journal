...
BUY_RULES = [
    ...
    "br1.5 IPO Base", "br1.6 Flat Base", "br1.7 Consolidation Pivot", # <--- ADDED
    # 2. Volume & Volatility
    ...
    "br10.9 Add: Upside Reversal", "br10.10 Add: Consolidation Pivot", # <--- ADDED
    # Misc
    "ns No Setup"
]
...
# ==============================================================================
# PAGE 1: COMMAND CENTER (3-COLUMN LAYOUT + 457B FIX)
# ==============================================================================
if page == "Command Center":
    st.title("COMMAND CENTER")

    # --- CONFIGURATION ---
    # Global Reset Date for Circuit Breakers
    ...
    # --- 3. RECALCULATE DD (POST-SPLIT RESET) ---
    # Apply 12/16 Reset to CanSlim
    if not df_cj_c.empty:
        df_post_c = df_cj_c[df_cj_c['Day'] > RESET_DATE]
        if not df_post_c.empty:
            hwm_c = df_post_c['End NLV'].max()
            curr_c = df_post_c['End NLV'].iloc[-1]
            can_dd = ((curr_c - hwm_c) / hwm_c) * 100 if hwm_c > 0 else 0.0
        else: can_dd = 0.0

    # Apply 12/16 Reset to 457B Plan
    if not df_rj_c.empty:
        df_post_r = df_rj_c[df_rj_c['Day'] > RESET_DATE]
        if not df_post_r.empty:
            hwm_r = df_post_r['End NLV'].max()
            curr_r = df_post_r['End NLV'].iloc[-1]
            ret_dd = ((curr_r - hwm_r) / hwm_r) * 100 if hwm_r > 0 else 0.0
        else: ret_dd = 0.0
...
# --- TAB 1: PILOT'S PANEL (CORRECTED MARKET WINDOWS) ---
    with tab_dash:
        ...
        # --- PRE-FETCH MARKET DATA (NASDAQ) FOR M-FACTOR ---
        # We need this for Col 2 and Col 3 to match M-Factor Tab
        try:
            ...
            # General Market Window Logic (Nasdaq > 21e)
        ...
        with col_1:
            ...
            try:
                # UPDATED: Using ^IXIC instead of QQQ
                q_hist = yf.Ticker("^IXIC").history(period="6mo")
                if not q_hist.empty:
                    ...
                    for i in range(len(q_hist)):
                        idx = -(i+1)
                        if q_hist.iloc[idx]['Low'] > q_hist.iloc[idx]['21EMA']: consecutive += 1
                        else: break
                    ...
                    # --- 1. MARKET WINDOW LOGIC (STRATEGY SPECIFIC) ---
                    # Window is OPEN if we are Invested OR if Buy Signal just triggered.
                    # Window is CLOSED if we are Cash and Scanning.
                    if tqqq_invested or buy_signal:
                        mw_state = "OPEN"
                        mw_color = "green"
                    else:
                        mw_state = "CLOSED"
                        mw_color = "red" # or orange
                    ...
                    if not tqqq_invested:
                        if buy_signal: 
                            st.success("✅ **BUY SIGNAL** (Enter TQQQ)")
                        elif consecutive >= 3: 
                            st.warning("⏳ **WAITING UP DAY**")
                        else:
                            st.info(f"🛡️ **CASH** (Scanning...)")
                    else:
                        is_below = curr_q < curr_ema
                        if is_below:
                            # Buffer based on the Low of the ^IXIC violation candle
        ...
        with col_2:
            ...
            # 1. MARKET WINDOW (Matches M-Factor / Nasdaq)
            st.markdown(f"**Market Window:** :{m_color_gen}[**{m_state_gen}**]")
            
            # 2. CIRCUIT RESET LINE
            ...
            if fuse_b > cb1:
                ...
                st.warning(f"⚠️ **LEVEL 1** (DD: {fuse_b:.2f}%)")
            elif cb2 >= fuse_b > cb3:
                st.error(f"🛑 **LEVEL 2** (DD: {fuse_b:.2f}%)")
            else:
                st.error(f"☠️ **LEVEL 3** (DD: {fuse_b:.2f}%)")
    ...
    if not os.path.exists(final_j_path): st.error("Journal missing.")
        ...
        if not df_j.empty:
            ...
            if not df_ytd.empty:
                ...
                # --- FIXED SPY CALCULATION ---
                # Check prior year close for true YTD anchor
                if 'SPY' in df_j.columns:
                    # Filter for days strictly BEFORE current year with valid SPY data
                    prior_year_data = df_j[(df_j['Day'].dt.year < curr_year) & (df_j['SPY'] > 0)]
                    
                    if not prior_year_data.empty:
                        # Use the closing price of the last trading day of previous year
                        start_s = prior_year_data['SPY'].iloc[-1]
                    elif not df_ytd.empty and not df_ytd['SPY'].eq(0).all():
                        # Fallback: First available day of current year (if no history)
                        start_s = df_ytd.loc[df_ytd['SPY'] > 0, 'SPY'].iloc[0]
                    else:
                        start_s = 0.0
                        
                    # Get current (last available) price
            ...
            c3.metric("YTD Return", f"{ytd_val:.2f}%", f"SPY {ytd_spy:+.2f}%") 
...
# ==============================================================================
    ...
    # 1. LOAD JOURNAL (For Historical NLV & Peak)
    # We still load the Journal locally for the historical chart data
    def clean_num_local(x):
        try:
            if isinstance(x, str):
                return float(x.replace('$', '').replace(',', '').replace('%', '').strip())
    ...
    if os.path.exists(p_path):
        ...
        if not df.empty and 'Day' in df.columns:
            ...
            if not df_active.empty:
                ...
                # ==========================================================
                # 3. CALCULATE LIVE RISK (LOGIC COPIED FROM TAB 7)
                # ==========================================================
                current_total_risk = 0.0
                current_unreal_pl = 0.0
                
                # We check if df_s (Strategies) and df_d (Daily) exist in globals
                # If they are not defined, we try to load them as a fallback
                local_df_s = locals().get('df_s', globals().get('df_s', pd.DataFrame()))
                local_df_d = locals().get('df_d', globals().get('df_d', pd.DataFrame()))
                
                # Fallback Load if globals are empty (Just in case)
                if local_df_s.empty:
                    try: 
                        s_temp = os.path.join(DATA_ROOT, portfolio, 'Strategies_Log.csv')
                        if os.path.exists(s_temp): local_df_s = pd.read_csv(s_temp)
                    except: pass
                if local_df_d.empty:
                    try: 
                        d_temp = os.path.join(DATA_ROOT, portfolio, 'Daily_Log.csv')
                        if os.path.exists(d_temp): local_df_d = pd.read_csv(d_temp)
                    except: pass

                # --- START TAB 7 MATH ---
                if not local_df_s.empty:
                    # Filter for OPEN trades
                    # Ensure case insensitivity for 'OPEN'
                    if 'Status' in local_df_s.columns:
                        local_df_s['Status'] = local_df_s['Status'].astype(str).str.strip().str.upper()
                        df_open = local_df_s[local_df_s['Status'] == 'OPEN'].copy()
                    else:
                        df_open = pd.DataFrame()
                    
                    if not df_open.empty:
                        # Clean Cols
                        for col in ['Avg_Entry', 'Shares', 'Total_Cost', 'Unrealized_PL']:
                            if col in df_open.columns:
                                df_open[col] = df_open[col].apply(clean_num_local)
                        
                        # Clean Daily Log for Lookups
                        if not local_df_d.empty and 'Stop_Loss' in local_df_d.columns:
                            local_df_d['Stop_Loss'] = local_df_d['Stop_Loss'].apply(clean_num_local)

                        # --- 1. DATA ENRICHMENT (Exact Copy) ---
                        def get_last_stop(tid):
                            try:
                                # Ensure ID matching is robust (str vs int)
                                rows = local_df_d[local_df_d['Trade_ID'].astype(str) == str(tid)]
                        ...
                        # TOTALS
                        current_total_risk = df_open['Risk $'].sum()
                        if 'Unrealized_PL' in df_open.columns:
                            current_unreal_pl = df_open['Unrealized_PL'].sum()

                # --- 4. DISPLAY HEADS UP DISPLAY (HEADERS) ---
                ...
                # TOTAL DEDUCTION FOR FLOOR (Risk + Unrealized)
                deduction_amt = current_unreal_pl + current_total_risk
                
                if dd_pct < 5:
                    col3.caption(f"Buffer: ${dist_l1:,.0f} to Level 1")
                
                col3.caption(f"Stop Floor Impact: -${deduction_amt:,.0f}")
                ...
                # CALCULATE FLOOR SERIES (NLV - (Risk + P&L))
                stop_out_floor_series = nlvs - deduction_amt
                ...
                ax.plot(dates, l1_series, color='#f1c40f', linewidth=1.5, alpha=0.8, label='L1: Caution (-5%)')
                ax.plot(dates, l2_series, color='#e67e22', linewidth=1.5, alpha=0.8, label='L2: Defense (-7%)')
                ax.plot(dates, l3_series, color='#c0392b', linewidth=2, alpha=0.8, label='L3: Cash (-10%)')
                
                # Plot Dynamic Floor
                ax.plot(dates, stop_out_floor_series, color='red', linestyle='--', linewidth=2, label=f'Stop-Out Floor')

                # Scaling
                min_view = min(l3_series.min(), stop_out_floor_series.min())
                max_view = hwm_series.max()
                ...
                st.info(f"No data available after reset date.")
        else:
            st.warning("Journal file empty.")
    else:
    ...
    if os.path.exists(SUMMARY_FILE):
        ...
        if not closed.empty:
            ...
            # --- SECTION 2: TICKER DEEP DIVE ---
            st.markdown("---")
            st.subheader("2. Ticker Deep Dive")
            
            target_ticker = st.selectbox("Select a Ticker to Analyze", sorted(closed['Ticker'].unique().tolist()))
            
            if target_ticker:
                t_df = closed[closed['Ticker'] == target_ticker].sort_values('Closed_Date', ascending=False)
                
                # Metrics using your specific columns
                t_count = len(t_df)
                ...
                st.write(f"**Trade History for {target_ticker}:**")
                # Showing columns that exist in your file
                st.dataframe(
...
# ==============================================================================
# PAGE 9: POSITION SIZER (CRASH FIX + LIFO LOGIC)
# ==============================================================================
    ...
    tab_new, tab_manage, tab_add, tab_trim = st.tabs(["🆕 Plan New Trade", "🔗 Manage Active Campaign", "➕ Add (Pyramid)", "✂️ Trim (Sell Down)"])
    ...
    # --- TAB 2: MANAGE ACTIVE CAMPAIGN (ROBUST LIFO) ---
    with tab_manage:
        ...
        if not df_s.empty:
            ...
            if not open_ops.empty:
                ...
                # --- CALCULATE LOCKED RISK (LIFO + ROBUST COLUMNS) ---
                def get_val(r, candidates, default=0.0):
                    for c in candidates:
                        if c in r.index and pd.notnull(r[c]): return float(r[c])
                    return default
                ...
                if not trxs.empty:
                    buys = trxs[trxs['Action'] == 'BUY'].copy()
                    sells = trxs[trxs['Action'] == 'SELL'].copy()
                    
                    if 'Date' in buys.columns: buys['Date'] = pd.to_datetime(buys['Date'], errors='coerce')
                    buys = buys.sort_values('Date', ascending=False) # LIFO
                    
                    total_sold = sells['Shares'].abs().sum() if not sells.empty else 0.0
                    
                    for _, b in buys.iterrows():
                        orig_shares = get_val(b, ['Shares'])
                        # Check explicit remaining
                        explicit = get_val(b, ['Remaining_Shares', 'Remaining'], default=-1)
                        
                        if explicit >= 0:
                            net_shares = explicit
                        else:
                            if total_sold >= orig_shares:
                                net_shares = 0
                                total_sold -= orig_shares
                            else:
                                net_shares = orig_shares - total_sold
                                total_sold = 0
                        
                        if net_shares > 0:
                            b_price = get_val(b, ['Amount', 'Price', 'Entry Price', 'Cost'])
                            if b_price == 0: b_price = avg_cost
                            b_stop = get_val(b, ['Stop_Loss', 'Stop'])
                            if b_stop == 0: b_stop = b_price
                            
                            r_risk = (b_price - b_stop) * net_shares
                            if r_risk > 0: locked_risk += r_risk
                ...
                c4.metric("Locked Risk", f"${locked_risk:,.2f}")
                
                def_pct = (db_budget / equity * 100) if equity > 0 else 0.50
                risk_pct_input = c4.number_input("Campaign Risk Budget (%)", value=float(def_pct), step=0.05, format="%.2f", help="Expand risk to finance add.")
                active_budget = equity * (risk_pct_input / 100)
                
                st.markdown("---"); st.markdown(f"#### 🪜 Scale-In Calculator")
                sc1, sc2, sc3 = st.columns(3)
                add_price = sc1.number_input("Add Price ($)", value=float(live_curr), step=0.1, key=f"mo_add_px_{sel_ticker}")
                new_chart_stop = sc2.number_input("New Chart Stop ($)", value=0.0, step=0.1, key=f"mo_new_stop_{sel_ticker}")
                target_mode_mo = sc3.select_slider("Target Total Position", options=list(size_map.keys()), value="Full (10%)", key=f"mo_target_size_{sel_ticker}")
                target_pct = size_map[target_mode_mo]

                if new_chart_stop > 0:
                    avail_cap = active_budget - locked_risk
                    risk_per_new = add_price - new_chart_stop
                    
                    if risk_per_new <= 0 or avail_cap <= 0: max_risk_shares = 0
                    else: max_risk_shares = math.floor(avail_cap / risk_per_new)
                    
                    target_val = equity * (target_pct / 100)
                    curr_val = curr_shares * add_price
                    needed_val = target_val - curr_val
                    desired_shares = math.floor(needed_val / add_price) if needed_val > 0 else 0

                    final_shares = min(desired_shares, max_risk_shares)
                    is_limited = max_risk_shares < desired_shares
                    
                    st.markdown("### 🎯 Execution")
                    k1, k2, k3 = st.columns(3)
                    k1.metric("Desired Add", f"{desired_shares} shs", f"To {target_pct}%")
                    k2.metric("Risk-Allowed Add", f"{max_risk_shares} shs", f"Cap: ${avail_cap:,.0f} Left")
                    
                    if desired_shares == 0: k3.metric("Action", "HOLD", "At Target")
                    elif is_limited: k3.metric("Action", f"BUY {final_shares}", f"⚠️ Capped by Risk", delta_color="inverse")
                    else: k3.metric("Action", f"BUY {final_shares}", "✅ Approved")

                    st.markdown("---")
                    new_total = curr_shares + final_shares
                    if new_total > 0:
                        new_cost = ((curr_shares * avg_cost) + (final_shares * add_price)) / new_total
                        proj_w = (new_total * add_price / equity) * 100
                        tot_risk = locked_risk + (final_shares * risk_per_new)
                        
                        p1, p2, p3 = st.columns(3)
                        p1.metric("New Total", f"{new_total} shs", f"Avg: ${new_cost:,.2f}")
                        p2.metric("Projected Weight", f"{proj_w:.1f}%", f"Target: {target_pct}%")
                        p3.metric("Total Risk Check", f"${tot_risk:,.2f}", f"vs Budget ${active_budget:,.2f}", delta_color="inverse" if tot_risk > active_budget + 5 else "normal")
                else: st.info("👈 Enter 'New Chart Stop'.")
            else: st.info("No active campaigns.")
        else: st.error("Summary file empty.")

    # --- TAB 3 & 4 (LEGACY) ---
    with tab_add:
        st.info("Please use 'Manage Active Campaign' for precision adding.")
    with tab_trim:
        ...
        if not open_positions.empty:
            ...
            if st.button("Calculate Trim", type="primary", key="trim_btn"):
                if target_weight_t >= curr_weight_t: st.warning(f"⚠️ Target ({target_weight_t}%) is higher than Current.")
                else:
                    ...
                    c_res1.metric("SELL SHARES", f"-{int(shares_to_sell)}")
                    c_res2.metric("REMAINING", f"{int(remaining_shares)}")
                    c_res3.metric("NEW WEIGHT", f"{actual_new_weight:.1f}%")
...
# ==============================================================================
    ...
    # Updated Tab List (Inserted Risk Manager at Index 7)
    tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab_risk, tab8, tab9, tab_cy, tab10, tab11 = st.tabs([
        "Log Buy", 
        "Log Sell", 
        "Update Prices", 
        "Edit Transaction", 
...
# ==============================================================================
# TAB 7: ACTIVE CAMPAIGN SUMMARY (AUTO-REFRESH LIVE PRICES)
# ==============================================================================
    with tab7:
        ...
        if not df_s.empty:
             ...
             if not df_open.empty:
                 ...
                 # --- 0. LIVE PRICE INJECTION (NEW) ---
                 # Get list of open tickers
                 tickers = df_open['Ticker'].unique().tolist()
                 live_prices = {}
                 ...
                 if tickers:
                     try:
                         # Fetch last close or live price
                         data = yf.download(tickers, period="1d", progress=False)['Close']
                         # Handle single vs multiple ticker result structure
                         if len(tickers) == 1:
                             val = data.iloc[-1]
                         ...
                         pass # Fallback to file prices if fetch fails

                 # Update Current Price in df_open
                 def get_live_price(row):
                     # Try live fetch first
                     if row['Ticker'] in live_prices:
                         return live_prices[row['Ticker']]
                     # Fallback 1: Calculate from Summary File (Value / Shares)
                     elif row['Shares'] > 0:
                         return (row['Total_Cost'] + row.get('Unrealized_PL', 0)) / row['Shares']
                     # Fallback 2: Avg Entry
                     return row['Avg_Entry']

                 df_open['Current Price'] = df_open.apply(get_live_price, axis=1)
                 
                 # RE-CALCULATE FINANCIALS WITH LIVE PRICE
                 df_open['Current Value'] = df_open['Shares'] * df_open['Current Price']
                 df_open['Unrealized_PL'] = df_open['Current Value'] - df_open['Total_Cost']
                 ...
                 def get_col_val(row, candidates, default=0.0):
                     for c in candidates:
                         if c in row.index and pd.notnull(row[c]): return float(row[c])
                     return default

                 # --- 1. RISK REPLAY ENGINE (TRUE LIFO + NETTING) ---
                 def calculate_net_risk_replay(row):
                     tid = row['Trade_ID']
                     trxs = df_d[df_d['Trade_ID'] == tid].copy()
                     if trxs.empty: return 0.0, 0.0
                     
                     if 'Date' in trxs.columns:
                         trxs['Date'] = pd.to_datetime(trxs['Date'], errors='coerce')
                         trxs = trxs.sort_values('Date', ascending=True)
                     
                     inventory = []
                     
                     for _, tx in trxs.iterrows():
                         action = tx.get('Action', 'BUY').upper()
                         shares = abs(get_col_val(tx, ['Shares']))
                         
                         if action == 'BUY':
                             price = get_col_val(tx, ['Amount', 'Price', 'Entry Price', 'Cost'])
                             if price == 0: price = row['Avg_Entry']
                             stop = get_col_val(tx, ['Stop_Loss', 'Stop', 'SL'])
                             if stop == 0: stop = price 
                             inventory.append({'shares': shares, 'price': price, 'stop': stop})
                             ...
                             for i in range(len(inventory) - 1, -1, -1):
                                 ...
                                 lot = inventory[i]
                                 if lot['shares'] > qty_to_sell:
                                     lot['shares'] -= qty_to_sell
                                     qty_to_sell = 0
                                 else:
                                     qty_to_sell -= lot['shares']
                                     inventory.pop(i)
                     
                     # --- CALCULATE NET CAMPAIGN RISK ---
                     net_risk_exposure = 0.0
                     ...
                     for lot in inventory:
                         ...
                         # Standard Risk Formula: Entry - Stop
                         tranche_risk = (r_price - r_stop) * r_shares
                         
                         net_risk_exposure += tranche_risk
                         weighted_stop_num += (r_stop * r_shares)
                         total_shares += r_shares
                     
                     # Net Risk Floor at 0
                     final_risk = max(0.0, net_risk_exposure)
                     avg_stop = weighted_stop_num / total_shares if total_shares > 0 else 0.0
                     
                     return final_risk, avg_stop

                 # --- APPLY LOGIC ---
                 risk_data = df_open.apply(calculate_net_risk_replay, axis=1, result_type='expand')
                 df_open['Risk $'] = risk_data[0]
                 df_open['Avg Stop'] = risk_data[1]

                 # Days Held
                 def get_days_held(tid):
                     try:
                         ...
                         if not rows.empty and 'Date' in rows.columns:
                             ...
                             if pd.notnull(start) and start <= now:
                                 return len(pd.bdate_range(start=start, end=now))
                         return 0
                     except: return 0
                 ...
                 def get_trend_status(row):
                     try:
                         # Use live price calculated above
                         live_p = row['Current Price']
                         hist = yf.Ticker(row['Ticker']).history(period="2mo")
                         if not hist.empty:
                             hist['21EMA'] = hist['Close'].ewm(span=21, adjust=False).mean()
                             live_ema = hist['21EMA'].iloc[-1]
                             if row['Avg_Entry'] < live_ema: return f"🏆 WON (Cost < 21e)"
                             return f"✅ > 21e" if live_p > live_ema else f"⚠️ < 21e"
                     except: return "N/A"
                     return "N/A"
                 ...
                 m6.metric("Total Heat", f"{df_open['Risk %'].sum():.2f}%")
                 
                 # --- 3. DATAFRAME ---
                 if 'Return_Pct' in df_open.columns:
                     df_open = df_open.sort_values(by='Return_Pct', ascending=False)
                 
                 cols = ['Trade_ID', 'Ticker', 'Days Held', 'Trend Status', 'Return_Pct', 'Pos Size %', 
                 ...
                 for _, r in df_open.iterrows():
                     # Only warn if Positive Risk > Budget
                     if r['Risk $'] > (r.get('Risk_Budget', 0) + 5):
                         st.warning(f"⚠️ **{r['Ticker']}**: Net Risk (${r['Risk $']:.0f}) > Budget. Raise Stop.")
                         all_clear = False
                     if r['Return_Pct'] <= -7.0:
...
# --- TAB RISK: RISK MANAGER (INTEGRATED) ---
    with tab_risk:
        ...
        # 1. LOAD JOURNAL (Only for Historical Chart & HWM)
        # We need the journal history to plot the Equity Curve vs Hard Decks
        p_path = os.path.join(DATA_ROOT, portfolio, 'Trading_Journal_Clean.csv')
        ...
        if os.path.exists(p_path):
            ...
            if not df_j.empty and 'Day' in df_j.columns:
                ...
                # Local helper to avoid cleaning errors
                def clean_num_rm(x):
                    try:
                        if isinstance(x, str):
                            return float(x.replace('$', '').replace(',', '').replace('%', '').strip())
                ...
                if not df_active.empty:
                    ...
                    # ==========================================================
                    # 2. CALCULATE LIVE RISK (USING GLOBAL df_s & df_d)
                    # ==========================================================
                    # This uses the data ALREADY loaded by Trade Manager
                    current_total_risk = 0.0
                    current_unreal_pl = 0.0
                    
                    if not df_s.empty:
                        # Filter for OPEN trades
                        # Normalize Status to ensure we catch 'Open ' vs 'OPEN'
                        ...
                        if not df_open.empty:
                            ...
                            for col in ['Avg_Entry', 'Shares', 'Total_Cost', 'Unrealized_PL']:
                                if col in df_open.columns:
                                    df_open[col] = df_open[col].apply(clean_num_rm)
                            
                            # Clean Daily Log for Lookups (df_d is available from parent scope!)
                            if not df_d.empty and 'Stop_Loss' in df_d.columns:
                                df_d['Stop_Loss_Clean'] = df_d['Stop_Loss'].apply(clean_num_rm)

                            # --- EXACT FUNCTION FROM TAB 7 ---
                            def get_last_stop_rm(tid):
                                try:
                                    # Ensure ID matching is robust (str vs int)
                                    rows = df_d[df_d['Trade_ID'].astype(str) == str(tid)]
                                    # Use the clean column
                                    valid_stops = rows[rows['Stop_Loss_Clean'] > 0.01]['Stop_Loss_Clean']
                                    return valid_stops.iloc[-1] if not valid_stops.empty else 0.0
                                except: return 0.0
                            
                            df_open['Stop Loss'] = df_open['Trade_ID'].apply(get_last_stop_rm)
                    ...
                    # --- 4. VISUALIZATION (HARD DECK CHART) ---
                    st.subheader("📉 The Hard Deck")
...
# ==============================================================================
# TAB 8: ACTIVE CAMPAIGN DETAILED (DYNAMIC FLIGHT DECK)
# ==============================================================================
    with tab8:
        ...
        if not df_d.empty and not df_s.empty:
            ...
            if not view_df.empty:
                ...
                # We calculate everything now so we can aggregate it for the Flight Deck
                
                # Maps for results
                remaining_map = {}
                lifo_pl_map = {} 
                
                # Global Aggregates for Flight Deck
                fd_realized_pl = 0.0
                ...
                for tid in target_df['Trade_ID'].unique():
                    ...
                    inventory = [] # [{'idx':..., 'qty':..., 'price':...}]
                    
                    for idx, row in subset.iterrows():
                        if row['Action'] == 'BUY':
                            # Get Price (Handle varies cols)
                            ...
                            fd_realized_pl += true_pl # Add to Flight Deck Total

                    # Add remaining inventory to Flight Deck Cost Basis
                ...
                if tick_filter != "All":
                    ...
                    # NOW ACCURATE: Sum of LIFO P&L from table
                    f5.metric("Realized P&L", f"${fd_realized_pl:,.2f}", delta_color="normal")
                    f6.metric("Total Equity", f"${mkt_val:,.2f}", f"{pos_size_pct:.1f}% Size")
                    st.markdown("---")

                # --- 4. FINALIZE TABLE DISPLAY ---
                # Unrealized P&L (Row Level)
